...
class MainWindow(baseClass):
    ...
    def v_read_convert_p(self):
        voltage = self.task.read()[1]
        m = 0.2684
        DC0 = -1.923
        V0 = -0.956
        A = 3.101
        t1 = 4.307
        c = -0.384
        eff_pow =m*(DC0+A*math.exp((voltage-V0)/t1))+c  
        return eff_pow
    ...
    def update_plot_data(self, time, pow_in, error): #Updates the plot with new values of time and pow_in. Plot resizes depending on 'window'
        ...
        if self.ui.inf_checkBox.isChecked() == True:
            ...
            if diff == 0:
                ...
                self.x = self.x[5:]  # Remove the first y element.
                self.x.append(time)  # Add a new value 1 higher than the last.
                self.y = self.y[5:]  # Remove the first
                self.y.append(pow_in)  # Add a new power value.
